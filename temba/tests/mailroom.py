...
def parse_location(org, location_string, level, parent=None):
    ...
    """
    # no country? bail
    if not org.locations_id or not isinstance(location_string, str):
        return []
...
def parse_location_path(org, location_string):
    ...
    return (
        ...
        if org.locations_id and isinstance(location_string, str)
        else None
    )
...
def find_boundary_by_name(org, name, level, parent):
    ...
    if parent:
        ...
        query["__".join(["parent"] * level)] = org.locations
