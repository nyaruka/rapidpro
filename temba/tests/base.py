...
class TembaTestMixin:
    ...
    def set_field(self, contact, user, key, value, label=None):
                ...
                if value is None:
                    ...
                    (
                        cursor.execute(
                            ...
                            [field_uuid, contact.modified_on, contact.id],
                        )
                    )
                else:
                    # update the field
                    (
                        cursor.execute(
                            ...
                            [json.dumps({field_uuid: contact.fields[field_uuid]}), contact.modified_on, contact.id],
                        )
                    )

        # update any groups or campaigns for this contact
        if has_changed:
            contact.handle_update(fields=[field.key])
