...
class URLResult(object):
    def __init__(self, url, times, allowed_max, prev_times):
        ...
        if prev_avg is not None:
            ...
            self.exceeds_change = (
                self.change > ALLOWED_CHANGE_MAXIMUM and self.percentage_change > ALLOWED_CHANGE_PERCENTAGE
            )
        else:
            self.change = None
            ...
            json.dump(
                {
                    ...
                    "results": [{"org": r["org"].id, "tests": [t.as_json() for t in r["tests"]]} for r in results],
                },
                f,
                indent=4,
            )
