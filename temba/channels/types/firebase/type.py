...
class FirebaseCloudMessagingType(ChannelType):
    ...
    configuration_blurb = _(
        ...
        <p>To register contacts, POST to the following URL with the parameters urn, fcm_token and optionally name.</p>

        <code>https://{{ channel.callback_domain }}{% url 'handlers.fcm_handler' 'register' channel.uuid %}</code>

        <h4>Receive URL</h4>

        <p>To handle incoming messages, POST to the following URL with the parameters from, msg and fcm_token.</p>

        <code>https://{{ channel.callback_domain }}{% url 'handlers.fcm_handler' 'receive' channel.uuid %}</code>
