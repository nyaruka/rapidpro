...
class TwilioTypeTest(TembaTest):
    ...
    def test_claim(self, mr_mocks):
        ...
        with patch("temba.channels.types.twilio.views.ClaimView.get_twilio_client") as mock_get_twilio_client:
            ...
            self.assertRedirects(
                response,
                f"{reverse('channels.types.twilio.connect')}?claim_type=twilio",
            )
        ...
        with patch("temba.tests.twilio.MockTwilioClient.MockPhoneNumbers.list") as mock_search:
            ...
            self.assertEqual(
                response.json(),
                ["+1 206-234-5678", "+1 206-234-5678", "+1 206-234-5678"],
            )
            ...
            self.assertEqual(
                response.json(),
                ["+44 28 1234 5678", "+44 28 1234 5678", "+44 28 1234 5678"],
            )
    ...
    def test_get_error_ref_url(self):
        self.assertEqual(
            ...
            TwilioType().get_error_ref_url(None, "30006"),
        )
            ...
            with patch("temba.tests.twilio.MockTwilioClient.MockAccounts.get") as mock_get_primary:
                with patch("twilio.rest.api.v2010.account.AccountContext.fetch") as mock_account_fetch:
                    ...
                    self.assertEqual(
                        ...
                        reverse("channels.types.twilio.claim"),
                    )

                    self.assertIn(TwilioType.SESSION_ACCOUNT_SID, self.client.session)
                    self.assertIn(TwilioType.SESSION_AUTH_TOKEN, self.client.session)
                    self.assertEqual(
                        self.client.session[TwilioType.SESSION_ACCOUNT_SID],
                        "AccountSid",
                    )
                    self.assertEqual(
                        self.client.session[TwilioType.SESSION_AUTH_TOKEN],
                        "PrimaryAccountToken",
                    )
                    ...
                    self.assertEqual(
                        response.request["PATH_INFO"],
                        reverse("channels.types.twilio_messaging_service.claim"),
                    )
                    ...
                    self.assertEqual(
                        response.request["PATH_INFO"],
                        reverse("channels.types.twilio_whatsapp.claim"),
                    )

                    response = self.client.post(
                        f"{reverse('channels.types.twilio.connect')}?claim_type=unknown",
                        post_data,
                        follow=True,
                    )
