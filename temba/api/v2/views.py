...
from temba.api.v2.views_utils import (
    BaseAPIView,
    BulkWriteAPIMixin,
...
class GroupsEndpoint(ListAPIMixin, WriteAPIMixin, DeleteAPIMixin, BaseAPIView):
    ...
    * **name** - the group name (string)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if triggers.count() > 0:
            raise InvalidQueryError(
                f"This group is used by active triggers. In order to delete it, first remove triggers: {', '.join(str(trigger) for trigger in triggers)}"
            )

        flows = Flow.objects.filter(org=instance.org, group_dependencies__in=[instance])
        if flows.count():
            raise InvalidQueryError(
                f"This group is used by active flows. In order to delete it, first remove flows: {', '.join(str(flow) for flow in flows)}"
            )

        campaigns = instance.campaign_set.filter(is_archived=False)
        if campaigns.exists():
            raise InvalidQueryError(
                f"This group is used by active campaigns. In order to delete it, first remove campaigns: {', '.join(str(campaign) for campaign in campaigns)}"
            )
