...
class AfricasTalkingHandler(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=AFRICAS_TALKING).exclude(org=None).first()
        if not channel:
...
class ZenviaHandler(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=ZENVIA).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel with uuid: %s not found." % channel_uuid, status=404)
...
class InfobipHandler(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=INFOBIP).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel with uuid: %s not found." % channel_uuid, status=404)
...
class HighConnectionHandler(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        channel_uuid = kwargs['uuid']
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=HIGH_CONNECTION).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel with uuid: %s not found." % channel_uuid, status=400)
...
class BlackmynaHandler(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        channel_uuid = kwargs['uuid']
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=BLACKMYNA).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel with uuid: %s not found." % channel_uuid, status=400)
...
class SMSCentralHandler(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        channel_uuid = kwargs['uuid']
        channel = Channel.objects.filter(uuid=channel_uuid, is_active=True,
                                         channel_type=SMSCENTRAL).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel with uuid: %s not found." % channel_uuid, status=400)
...
class NexmoHandler(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # look up the channel
        channel = Channel.objects.filter(uuid=channel_number, is_active=True,
                                         channel_type=NEXMO).exclude(org=None).first()
...
class VerboiceHandler(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        channel = Channel.objects.filter(uuid__iexact=request_uuid, is_active=True,
                                         channel_type=VERBOICE).exclude(org=None).first()
        if not channel:
...
class KannelHandler(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        channel = Channel.objects.filter(uuid=request_uuid, is_active=True,
                                         channel_type=KANNEL).exclude(org=None).first()
        if not channel:
...
class ClickatellHandler(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # look up the channel
        channel = Channel.objects.filter(uuid=request_uuid, is_active=True,
                                         channel_type=CLICKATELL).exclude(org=None).first()
        if not channel:
            return HttpResponse("Channel not found for id: %s" % request_uuid, status=400)
