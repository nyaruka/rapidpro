...
class WebHookEvent(SmartModel):
    ...
    def trigger_sms_event(cls, event, msg, time):
        ...
        if (
            (event == cls.TYPE_SMS_RECEIVED and not org.is_notified_of_mo_sms()) or
            (event == cls.TYPE_SMS_SENT and not org.is_notified_of_mt_sms()) or
            (event == cls.TYPE_SMS_DELIVERED and not org.is_notified_of_mt_sms())
        ):
            return
