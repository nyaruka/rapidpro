...
def add_deps(Flow, ActionSet, RuleSet):
    ...
    logger.debug("Collecting flows and dependencies...")
    ...
    logger.debug("Processing flow dependencies...")
    for flow_ids_chunk in chunk_list(flow_ids, 1000):
        ...
        logger.debug(f"Processed {processed_flows}/{total_flows} in {time.monotonic() - chunk_start_time}")

    logger.debug(f"Collected flows and dependencies in {time.monotonic() - start_time}")

    logger.debug("Comparing actual to expected flow dependencies...")
    ...
    logger.debug(f"Total added missing deps: {total_added_deps}")
