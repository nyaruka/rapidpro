...
class FlowEnteredTest(TembaTest):
    def test_trigger_description(self):
        tests_data = [
            dict(
        ...
        for t_dict in tests_data:
            self.assertEqual(
                FlowEntered({}, t_dict["session_output_trigger"]).trigger_description, t_dict["trigger_description"]
            )
