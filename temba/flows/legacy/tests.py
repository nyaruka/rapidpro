...
class FlowMigrationTest(TembaTest):
    ...
    def test_migrate_to_11_6(self):
        ...
        for uuid, name in migrated_groups.items():
            self.assertTrue(ContactGroup.groups.filter(uuid=uuid, name=name).exists(), msg="Group UUID mismatch")
    ...
    def test_migrate_bad_group_names(self):
        ...
        for v in ("4", "5", "6", "7", "8", "9", "10"):
            ...
            self.assertTrue(ContactGroup.groups.filter(name="Contacts < 25").exists(), error % ("< 25", v))
            self.assertTrue(ContactGroup.groups.filter(name="Contacts > 100").exists(), error % ("> 100", v))
    ...
    def test_migrate_malformed_groups(self):
        ...
        self.assertTrue(ContactGroup.groups.filter(name="Contacts < 25").exists())
        self.assertTrue(ContactGroup.groups.filter(name="Unknown").exists())
