        ...
        def get_context_data(self, *args, **kwargs):
            ...
            while True:
                history += Event.get_by_contact(
                    contact, self.request.user, after, fetch_before, ticket_uuid=ticket_uuid, limit=limit
                )
            ...
            if not recent_only and before > contact.created_on:
                context["has_older"] = bool(
                    Event.get_by_contact(
                        contact, self.request.user, contact_creation, after, ticket_uuid=ticket_uuid, limit=1
                    )
                )
