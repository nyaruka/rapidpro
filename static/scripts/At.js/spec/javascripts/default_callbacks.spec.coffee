...
describe "default callbacks", ->
  ...
  it "should match if escape char omitted", ->
    $inputor = $("#escape-char").atwho at: "@", data: fixtures["names"]
    text = $.trim $inputor.text()
    query = callbacks.matcher.call(app, "@", text)
    expect(query).toBe("Jobs")

  it "should not match if following escape char", ->
    $inputor = $("#escape-char").atwho at: "@", data: fixtures["names"]
    text = $.trim $inputor.text()
    query = callbacks.matcher.call(app, "@", text, "@")
  ...
  it "should not match a space following @ if acceptSpaceBar flag false", ->
    ...
    text = $.trim $inputor.text()
    query = callbacks.matcher.call(app, "@", text, undefined, false, false)
    expect(query).toBe("Jobs")
  ...
  it "should match a space following @ if acceptSpaceBar flag set to true", ->
    ...
    text = $.trim $inputor.text()
    query = callbacks.matcher.call(app, "@", text, undefined, false, true)
